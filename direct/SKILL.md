...
- make Codex full access without command approvals the default Paseo mode;
...
For Codex, verify that `~/.codex/config.toml` contains top-level `sandbox_mode = "danger-full-access"` and `approval_policy = "never"`, while `[features] default_mode_request_user_input = true`. Also verify that the installed Paseo Codex provider reports `full-access` as its default mode. Paseo 0.4.0 otherwise sends `workspace-write` with its `auto-review` mode and overrides Codex's file-level sandbox setting. Command approvals and Codex's user-question tool are separate: keep the latter enabled.
